...
def fp8_matmul_npu(
    ...
    if _is_npu_before_atlas_a5:
        while input.dim() > 2 and input.size(0) == 1:
        ...
        output = torch.ops.npu.npu_quant_matmul(x, weight, scale=weight_scale, pertoken_scale=x_scale, output_dtype=torch.bfloat16, group_sizes=group_sizes)
