...
def attention_sinks_prefill_kernel(
    ...
    q_start_offset = tl.load(cum_seq_lens + i_b)
    q_end_offset = tl.load(cum_seq_lens + i_b + 1)
    ...
    for i_s in range(q_start_offset, q_end_offset, Br):
        ...
        if sliding_window_size != -1:
            ...
            start_kv_len_block = max((kv_seq_len_block - sliding_window_size).to(tl.int32), 0)
        ...
        q = tl.load(query + offset_seq[:, None] + offset_q[None, :], mask=mask_seq[:, None]).to(tl.float32)
